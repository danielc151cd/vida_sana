...
// Traer sucursales
$sucursales_res = pg_query($conn, "SELECT id_sucursal, nombre_sucursal FROM sucursales ORDER BY nombre_sucursal ASC");
$sucursales = [];
while($row = pg_fetch_assoc($sucursales_res)){
    $sucursales[] = $row;
}
...
<head>
    ...
    <title>Transferencias (Admin) - Sistema Farmacia</title>
    <link rel="stylesheet" href="css/transferencias-elegant.css">
</head>
<body>
<div class="container">
    <h2>Transferencias (Administrador)</h2>
    ...
    <hr>
    <h3>📋 Últimas transferencias registradas</h3>
    <?php
    $sql_hist = "SELECT t.numero_transferencia,
                 ...
                 ORDER BY fecha DESC
                 LIMIT 20";
    $res_hist = pg_query($conn, $sql_hist);
    ...
    if ($res_hist && pg_num_rows($res_hist) > 0) {
        ...
        echo "</table>";
        echo "<br><button onclick=\"window.location.href='export_transferencias_admin.php'\">⬇ Exportar todas en Excel</button>";
    } else {
        echo "<p>❌ No hay transferencias registradas aún.</p>";
    }
    ?>
</div>
